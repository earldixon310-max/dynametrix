...
Outcome document: `analysis/sst2_negation_ovp/RESULT_SST2_OVP_CALIB.md` (citable only after its own cold cross-pass). Any continuation (feature decompression, floor-rule revision, larger substrate, or substrate abandonment) is a **new pre-registration**. The §6 ladder is unmoved (no ledger verdict, as pre-registered).
