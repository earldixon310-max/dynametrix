...
```
python3 test_ovp_guard.py      # 20 unit (closed-world open/os.open/listdir/socket/subprocess/os.system/numpy; absent-path; empty-input)
python3 test_integration.py    # 7 end-to-end (smoke; FULL chain across lock; post-lock core/.gitattributes/input/GUARD/JUDGE edits REFUSE)
...
```
37 properties total, all asserted by execution (rev7). (Confirm by *running* — don't trust a stated count.)
...
- **Built + tested (37 properties):** H1 over **all** sealed-path sources **including `.gitattributes`** (rev4), with post-lock edits of the **guard module and the judge itself** now pinned by shipped tests (rev7); the full closure chain run end-to-end as one coupled unit (H1 + output-exists + input-hash + closed-world); the `(sealed-loader, synthetic-loader, shared-core)` factoring; **the §3 signed additive attestation tool** (binding, anchoring, grade, and temporal all **re-derived, never trusted**). `emit_sign_command` is exercised at the **command-string** level (test J); the actual `git tag -s` / `git tag -v` signing is the operator's R-1 human-trust step, **not** tested here.
- **Closed-world: the `open` allowlist is the provable guarantee; spawn/network/enumeration are best-effort (rev5–6, cold-pass-A readers #4/#5).** The **allowlist on `open`** is genuinely deny-by-default — any non-allowlisted file read refuses, including `os.open` and **numpy's readers** (`np.load` of a non-allowlisted path refuses; tested, test 18). **`ctypes` `dlopen` does NOT route through `open`** — it is the on-ramp to the C-level libc residual and is **uncovered**. Enumeration/network/spawn are denied via a **denylist of only confirmed-live audit events** (`os.listdir/scandir`, `socket.connect/getaddrinfo`, `subprocess.Popen`, `os.system`, `os.posix_spawn`, `os.exec`, `shutil.copyfile` — each verified to fire and refuse). A denylist **cannot be exhaustive** (the spec warned this). Dead strings have been pruned: rev5's `os.spawnv` and rev6's `os.popen` both fired *no* matching event (`os.popen` routes through `subprocess.Popen`, already covered) and were removed so the list is truthfully "confirmed-live." `os.spawnv`'s coverage is **platform/version-dependent** (on CPython the fork-child inherits this hook and its `exec` is denied, so it is often caught — but this is not guaranteed, so it is treated as uncovered/best-effort, the conservative direction); `os.fork` without exec is *bounded* because the child inherits this hook. **The real closure for spawn/network/`dlopen` exfiltration is the operator's external sandbox** (no-network, restricted process) for the canonical run; the audit denylist is defense-in-depth, not a guarantee.
...
**rev7 — adoption pass 2 (executing).** Pass 1 was CLEAN; pass 2 found one class-(b) blocker: **`os.popen` was a dead string** in `_DENY_EVENTS` (CPython fires only `subprocess.Popen`; the list claimed "only confirmed-live") — the same defect class as rev6's `os.spawnv`, so by precedent it **resets**. Removed (coverage retained via `subprocess.Popen`); every remaining entry verified live. Because the reset was *forced*, the queue-don't-fold rule no longer pinned the non-blockers, so rev7 also folded everything both passes flagged: the `os.spawnv` reasoning corrected (platform-dependent, not "parent doesn't observe"); the previously-untested `os.open` and isolated-`socket.connect` now tested (guard 19/20); the **guard-module and judge self-edit** cases pinned by shipped tests (integration 20/21); and the rev7 spec §1/§4 "five-file" stale wording reconciled to §2's "every sealed-path source." **37/37; the adoption count resets — two fresh executing passes on rev7.**
