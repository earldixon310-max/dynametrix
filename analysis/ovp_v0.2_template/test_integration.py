...
sh(["git","checkout","--","Bdata.bin"],d)   # RESTORE input: tests 20/21 must isolate H1, not the input-hash gate
...
r=run(); check("20 post-lock ovp_guard.py edit REFUSES at H1 (message names it)", r.returncode==2 and "ovp_guard.py bytes !=" in r.stderr and not os.path.exists(os.path.join(d,"out.json")))
...
r=run(); check("21 post-lock judge_y.py edit REFUSES at H1 (message names it)", r.returncode==2 and "judge_y.py bytes !=" in r.stderr and not os.path.exists(os.path.join(d,"out.json")))
