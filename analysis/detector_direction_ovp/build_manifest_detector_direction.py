#!/usr/bin/env python3
"""
build_manifest_detector_direction.py - materialization manifest generator for
DETECTOR_DIRECTION_OVP (OVP real candidate #3). Run at LOCK time.
...
calibration result). Writes materialization_manifest_detector_direction.json.

Everything (B, y, predicted_prob_ai, pred) is inherited from the calibration-locked
per-example; there is NO new materialization (no tokenizer, no model run).
"""
import hashlib
import json
import os
...
ARTIFACTS = ["PRE_REGISTRATION_DETECTOR_DIRECTION_OVP.md", "judge_direction.py"]
MASTER_SEED = "0xDEC0DE"
JUDGE_SCRIPT = "judge_direction.py"
...
def main():
    ...
    if not (mlo and mhi and float(mlo.group(1)) == TAU_LO and float(mhi.group(1)) == TAU_HI):
        raise SystemExit("ABORT: cut-point mismatch between manifest and judge_direction.py; do not lock.")
    ...
    manifest = {
        "framework": "AEPF DETECTOR_DIRECTION_OVP materialization manifest",
        "study": "DETECTOR_DIRECTION_OVP real candidate study (#3): does directional predicted_prob_ai add HDG beyond folded confidence",
        "governing_spec": "OVP v0.1 @ signed tag ovp-v0.1-lock",
        "lock_tag": "detector-direction-ovp-lock",
        "generated_utc": datetime.now(timezone.utc).isoformat(),
        "master_seed": MASTER_SEED,
        "candidate": "predicted_prob_ai", "baseline": "confidence (folded max(p,1-p))",
        "inherited_cut_points": {"tau_lo": TAU_LO, "tau_hi": TAU_HI, "provenance": CUT_POINT_PROVENANCE,
                                 "identity_checked_at_lock": True},
        "substrate": {
            "type": "ALL of B,y,predicted_prob_ai,pred inherited from the calibration lock; NO new materialization; external model+dataset pinned for provenance",
            ...
            "attestation": ("B,y,predicted_prob_ai,pred are inherited byte-identical from the calibration-locked "
                            "per-example (hash-verified). No tokenizer, no model inference, no new materialization. "
                            "predicted_prob_ai's HDG is computed only by the single locked run of judge_direction.py."),
        },
        "environment": {
        ...
        "note": ("The pre-registration SHA-256 is the authoritative record of all pinned parameters. "
                 "transformers/torch are NOT required at runtime (no materialization); numpy + scikit-learn "
                 "govern the verdict."),
    }
    with open("materialization_manifest_detector_direction.json", "w") as f:
