#!/usr/bin/env python3
"""
build_manifest_detector_length.py - materialization manifest generator for
DETECTOR_LENGTH_OVP (OVP real candidate #2). Run at LOCK time.

Aborts the lock on: inherited per-example-hash mismatch; dataset-hash mismatch; 3-way
model-revision identity mismatch (audit model_revision.txt == manifest == script); or
cut-point-identity mismatch (script TAU vs frozen calibration result). Writes
materialization_manifest_detector_length.json for the single atomic lock commit.

text_length is materialized fresh by the single locked run (tokenizer only, no model
inference) and cross-checked against the inherited `truncated`; detector_length_per_example.csv
is therefore an OUTPUT of the run, not a lock input.
"""
import hashlib
import json
import os
...
ARTIFACTS = ["PRE_REGISTRATION_DETECTOR_LENGTH_OVP.md", "judge_length.py"]
MASTER_SEED = "0x73C0DE"
JUDGE_SCRIPT = "judge_length.py"
...
def main():
    ...
    if not (mlo and mhi and float(mlo.group(1)) == TAU_LO and float(mhi.group(1)) == TAU_HI):
        raise SystemExit("ABORT: cut-point mismatch between manifest and judge_length.py; do not lock.")
    # cross-check the manifest's pin against the actual calibration result too
    ...
    manifest = {
        "framework": "AEPF DETECTOR_LENGTH_OVP materialization manifest",
        "study": "DETECTOR_LENGTH_OVP real candidate study (#2): does `text_length` add HDG beyond confidence",
        "governing_spec": "OVP v0.1 @ signed tag ovp-v0.1-lock",
        "lock_tag": "detector-length-ovp-lock",
        "generated_utc": datetime.now(timezone.utc).isoformat(),
        "master_seed": MASTER_SEED,
        "candidate": "text_length", "baseline": "confidence (max softmax)",
        "inherited_cut_points": {"tau_lo": TAU_LO, "tau_hi": TAU_HI, "provenance": CUT_POINT_PROVENANCE,
                                 "identity_checked_at_lock": True},
        "substrate": {
            "type": "B,y,truncated inherited from the calibration lock; text_length materialized fresh (tokenizer only); external model+dataset pinned",
            ...
            "model": {"id": MODEL_ID, "revision": MODEL_REVISION, "tokenizer_only_for_length": True,
                      "identity_checked_at_lock": True},
            "attestation": ("B,y,truncated inherited byte-identical from the calibration-locked per-example "
                            "(hash-verified). text_length is materialized by the single locked run via the pinned "
                            "tokenizer (no model inference) and cross-checked: 1[text_length>512] must equal the "
                            "inherited truncated column or the run aborts. detector_length_per_example.csv is an "
                            "output of the run."),
        },
        "environment": {
            "python": sys.version.split()[0], "platform": platform.platform(),
            "numpy": version("numpy"), "scikit_learn": version("sklearn"), "transformers": version("transformers"),
        },
        ...
        "note": ("The pre-registration SHA-256 is the authoritative record of all pinned parameters. "
                 "transformers is required at runtime for the tokenizer only (text_length); torch/model "
                 "inference is NOT used; numpy + scikit-learn govern the verdict."),
    }
    with open("materialization_manifest_detector_length.json", "w") as f:
