...
A Framework for Locked Evaluation, Calibration Assessment,\\
and Reproducible Verification in Evaluation-Intensive\\
...
computational domains. AEPF integrates established disciplines
from clinical-trial pre-registration, high-energy-physics blind
analysis, machine-learning calibration assessment, and
open-science null reporting into a single operational framework.
The integration produces specific operational consequences:
cryptographic anchoring of methodology to a public commit chain,
calibration assessment as a standing pre-registration requirement,
null-result publication parity with positive-result discipline,
constrained-interpretation reporting when multiple frames apply,
and---as a refinement under observation---a pre-specification
viability gate that retires candidate studies whose informational
yield is structurally over-determined. The framework has been
applied across atmospheric forecasting calibration,
gravitational-wave residual analysis, language-model calibration
audits, cosmological residual-structure evaluation, and
framing-robustness testing of an open-weight language model. We
describe the framework specification and illustrate it with four
case studies spanning substantively different evaluation
contexts---a cosmological residual audit, a series of transformer
calibration audits, a pre-registered open-weight language-model
null, and a pre-specification viability gate---together with a
methods-level validation of the calibration-audit instrument
itself against synthetic ground truth. The instrument validation
establishes that the calibration audit is sound (zero false-accept
of broken calibration across the tested miscalibration families),
so that the framework's argument rests not only on disciplined
null-handling but also on demonstrated instrument soundness: a NULL
verdict reported under AEPF is evidence about the world rather than
an artifact of the tool. The novelty AEPF claims is in the
integration and artifact-level operationalization, not in any
